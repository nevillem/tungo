...
class DiseaseDetailScreen extends StatefulWidget {
  final CropData? diseaseItem;
  final String? image;
  final int curIndex;
  const DiseaseDetailScreen({Key? key, required this.diseaseItem,
...
class StateItem extends State<DiseaseDetailScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    int? _infoItemCount=widget.diseaseItem?.cropdiseases?.length;
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(
        titleSpacing: 0,
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: PageView.builder(
          itemCount: widget.diseaseItem?.cropdiseases?.length,
          ...
          itemBuilder: (BuildContext context, int index) {
            String numberinfo="${widget.diseaseItem?.cropdiseases![index]?.diseaseDetails!.length}";
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  SizedBox(
                      width: double.infinity,
                      child: Text("${widget.diseaseItem?.cropdiseases?[index].disease}",
                  ...
                  ListView.builder(
                      ...
                      itemCount:widget.diseaseItem?.cropdiseases![index].diseaseDetails!.length,
                      itemBuilder: (context, int columnno) {
                        return HtmlWidget("${widget.diseaseItem?.cropdiseases![index].diseaseDetails?[columnno].details}");
                      }):Center(
                      child: Text(
