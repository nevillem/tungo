...
class InfoItemDetails extends StatefulWidget {
  final CropData? infoItems;
  final String? image;
  final int curIndex;
  const InfoItemDetails({Key? key, required this.infoItems, required this.image, required this.curIndex}) : super(key: key);

  @override
  State<StatefulWidget> createState() => StateItem();
}

class StateItem extends State<InfoItemDetails> with TickerProviderStateMixin {
...
@override
  ...
  Widget build(BuildContext context) {
    int? _infoItemCount=widget.infoItems?.informationitems?.length;
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(
        titleSpacing: 0,
  ...
  _showContentInfoItem(){
    ...
    child: SizedBox(
      ...
      child: PageView.builder(
        itemCount: widget.infoItems?.informationitems?.length,
        ...
        itemBuilder: (BuildContext context, int index) {
          String numberinfo="${widget.infoItems?.informationitems![index]?.information!.length}";
          return Padding(
            ...
            child: Column(
              ...
              children: [
                SizedBox(
                  width: double.infinity,
                    child: Text("${widget.infoItems?.informationitems?[index].name}",
                ...
                ListView.builder(
                    ...
                    itemCount:widget.infoItems?.informationitems![index].information!.length,
                    itemBuilder: (context, int columnno) {
                      return HtmlWidget("${widget.infoItems?.informationitems![index].information?[columnno].information}");
                    }):Center(
                    child: Text(
